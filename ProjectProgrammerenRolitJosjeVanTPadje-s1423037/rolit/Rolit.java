	...
	public static void main(String[] args){
		if (args.length == Game.NUMBER_PLAYERS){
			...
			if (2 == Game.NUMBER_PLAYERS){
				Player s0 = createPlayer(args[0], Mark1.ROOD);
				Player s1 = createPlayer(args[1], Mark1.GROEN);
			}
			if (3 == Game.NUMBER_PLAYERS){
				Player s0 = createPlayer(args[0], Mark1.ROOD);
				Player s1 = createPlayer(args[1], Mark1.GEEL);
				Player s2 = createPlayer(args[2], Mark1.GROEN);
			}
			if (4 == Game.NUMBER_PLAYERS){
				Player s0 = createPlayer(args[0], Mark1.ROOD);
				Player s1 = createPlayer(args[1], Mark1.GEEL);
				Player s2 = createPlayer(args[2], Mark1.GROEN);
				Player s3 = createPlayer(args[3], Mark1.BLAUW);
			}
			Game spel = new Game();
			spel.start();
		}
